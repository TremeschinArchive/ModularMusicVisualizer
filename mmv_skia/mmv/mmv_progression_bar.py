...
class MMVProgressionBarVectorial:
    """
    kwargs:
    {
        "type": Determines what class of Progression Bar we'll use
            "rectangle": MMVProgressionBarRectangle
    ...
    def __init__(self, mmv, **kwargs) -> None:
        ...
        debug_prefix = "[MMVProgressionBarVectorial.__init__]"
        ...
        if self.config["type"] == "rectangle":
            print(debug_prefix, "Builder is MMVProgressionBarRectangle")
            self.builder = MMVProgressionBarRectangle(self, self.mmv)
